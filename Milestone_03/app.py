...
def get_simulation_report_metrics(history_entries):
    total_simulations = len(history_entries)
    unique_scenarios = len(set(h.scenario for h in history_entries))
    low_risk_count = sum(1 for h in history_entries if h.result_risk and h.result_risk.lower() == 'low')
    ...
    for h in history_entries:
        label = scenario_map.get(h.scenario, h.scenario)
        counts[label] += 1
        totals[label] += h.amount

    most_common_key = max(counts, key=counts.get) if counts else 'N/A'
    most_common_letter = most_common_key[0].upper() if most_common_key != 'N/A' else '-'

    chart_labels = list(scenario_map.values())
    chart_counts = [counts[l] for l in chart_labels]
    chart_avgs = [round(totals[l] / counts[l], 1) if counts[l] > 0 else 0 for l in chart_labels]

    return {
        'total': total_simulations,
        'unique': unique_scenarios,
        'most_common': most_common_letter,
        'most_common_full': most_common_key,
...
def user_simulation():
    ...
    if request.method == 'POST':
        ...
        report_metrics = get_simulation_report_metrics(history_entries)
...
@app.route('/user/simulation/delete/<int:id>', methods=['POST'])
@login_required
def delete_simulation_history(id):
    entry = SimulationHistory.query.filter_by(id=id, user_id=current_user.id).first()
    if entry:
        db.session.delete(entry)
        db.session.commit()
        flash('Simulation history entry deleted.', 'info')
    else:
        flash('Simulation history entry not found.', 'danger')
    return redirect(url_for('user_simulation'))
